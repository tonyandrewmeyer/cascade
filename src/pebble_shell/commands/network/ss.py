...
Note: This command shows information from the remote container environment.
    ...
    def execute(self, client: ops.pebble.Client | shimmer.PebbleCliClient, args: list[str]):
        ...
        if flags["K"] or flags["kill"]:
            self.shell.console.print(
                "[red]ss: socket killing not supported in container environment[/red]"
            )
            return 1

        if flags["E"] or flags["events"]:
            self.shell.console.print(
                "[red]ss: event monitoring not supported in container environment[/red]"
            )
            return 1

        if flags["N"] or flags["net"]:
            self.shell.console.print(
                "[yellow]ss: network namespace switching not supported in container environment[/yellow]"
            )
            return 0

        if flags["Z"] or flags["context"] or flags["z"] or flags["contexts"]:
            self.shell.console.print(
                "[yellow]ss: SELinux context display not available in container environment[/yellow]"
            )
            return 0
    ...
    def _show_summary(self, client, flags: dict) -> int:
        ...
        try:
            ...
            self.shell.console.print("RAW:   0")
            self.shell.console.print("FRAG:  0")
    ...
    def _show_sockets(self, client, protocols: list[str], flags: dict) -> int:
        ...
        flags["n"] or flags["numeric"]
        flags["e"] or flags["extended"]
        flags["H"] or flags["no-header"]
        ...
        for protocol in protocols:
            try:
                ...
                if flags["v"] or flags["verbose"]:
                    self.shell.console.print(
                        f"[yellow]ss: could not read {protocol} sockets[/yellow]"
                    )
    ...
    def _display_table(self, sockets: list[dict], flags: dict):
        ...
        table = Table(show_header=not (flags["H"] or flags["no-header"]), header_style="bold blue")
