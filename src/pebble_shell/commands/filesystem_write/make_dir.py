...
class MakeDirCommand(Command):
    ...
    def execute(self, client: ops.pebble.Client | shimmer.PebbleCliClient, args: list[str]):
        ...
        if handle_help_flag(self, args):
            self.show_help()
            return 0
        if not validate_min_args(self.shell, args, 1, "mkdir [-p] <directory1> [directory2...]"):
            return 1
        ...
        directories = [
            resolve_path(self.shell.current_directory, d, self.shell.home_dir) for d in directories
        ]
