"""Integration tests for the Dos2unixCommand."""
...
def command(shell: pebble_shell.shell.PebbleShell):
    """Fixture to create a Dos2unixCommand instance."""
    yield pebble_shell.commands.Dos2unixCommand(shell=shell)


def test_name(command: pebble_shell.commands.Dos2unixCommand):
    assert command.name == "dos2unix"


def test_category(command: pebble_shell.commands.Dos2unixCommand):
    assert command.category == "Text Processing"


def test_help(command: pebble_shell.commands.Dos2unixCommand):
...
def test_execute_help(
    ...
    command: pebble_shell.commands.Dos2unixCommand,
    args: list[str],
):
    with command.shell.console.capture() as capture:
...
def test_execute_no_args_stdin(
    ...
    # dos2unix with no args should read from stdin
    with command.shell.console.capture() as capture:
    ...
    if result == 0:
        ...
        assert result == 1
        output = capture.get()
        assert any(msg in output for msg in ["usage", "Usage", "dos2unix", "file"])


def test_execute_convert_dos_to_unix_file(
    client: ops.pebble.Client,
    command: pebble_shell.commands.Dos2unixCommand,
):
    # Test converting DOS line endings to Unix
    with command.shell.console.capture() as capture:
    ...
    if result == 0:
        output = capture.get()
        # Should contain file contents with Unix line endings
        assert len(output) >= 0
        # Should not contain DOS line endings (CRLF)
        if len(output) > 0:
            assert "\r\n" not in output
...
def test_execute_new_file_mode(
    ...
    with tempfile.NamedTemporaryFile(suffix="_unix", delete=False) as output_file:
    ...
    if result == 0:
        output = capture.get()
        # Should create new file with Unix line endings
        assert len(output) >= 0
    else:
        assert result == 1
...
def test_execute_file_with_unix_endings(
    client: ops.pebble.Client,
    command: pebble_shell.commands.Dos2unixCommand,
):
    # Test file that already has Unix line endings
    with command.shell.console.capture() as capture:
    ...
    if result == 0:
        output = capture.get()
        # Should pass through unchanged if already Unix format
        assert len(output) >= 0
        if len(output) > 0:
            # Should not contain DOS line endings
            assert "\r\n" not in output
    else:
        assert result == 1
...
def test_execute_file_with_mixed_endings(
    ...
    if result == 0:
        output = capture.get()
        # Should normalize all line endings to Unix format
        assert len(output) >= 0
        if len(output) > 0:
            # Should not contain DOS line endings
            assert "\r\n" not in output
            # Should contain only Unix line endings
            if "\n" in output:
