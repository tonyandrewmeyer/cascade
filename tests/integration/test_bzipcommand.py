"""Integration tests for the BzipCommand."""
...
def command(shell: pebble_shell.shell.PebbleShell):
    """Fixture to create a BzipCommand instance."""
    yield pebble_shell.commands.BzipCommand(shell=shell)


def test_name(command: pebble_shell.commands.BzipCommand):
    assert command.name == "bzip2"


def test_category(command: pebble_shell.commands.BzipCommand):
    assert command.category == "Compression"


def test_help(command: pebble_shell.commands.BzipCommand):
...
def test_execute_help(
    ...
    command: pebble_shell.commands.BzipCommand,
    args: list[str],
):
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=args)
    assert result == 0
    assert "bzip2" in capture.get()
...
def test_execute_with_decompress_flag(
    ...
    # Should fail since /etc/passwd is not a bzip2 file
    assert result == 1
