...
def test_execute_missing_dest_path(
    ...
    output = capture.get()
    assert any(
        msg in output for msg in ["missing destination", "push <source-path> <dest-path>", "usage"]
    )
...
def test_execute_simple_file_push(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/passwd"])
    ...
    if result == 0:
        ...
        assert any(
            msg in output for msg in ["File pushed successfully", "Transfer completed", "uploaded"]
        )
    else:
...
def test_execute_create_dirs_option(
    ...
    if result == 0:
        ...
        assert any(msg in output for msg in ["File pushed successfully", "Transfer completed"])
    else:
        assert result == 1
...
def test_execute_invalid_permissions(
    ...
    assert result == 1
    output = capture.get()
    assert any(msg in output for msg in ["invalid permissions", "Push operation failed", "error"])
...
def test_execute_overwrite_existing_file(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/existing-file"])
...
def test_execute_permission_denied_source(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/root/.ssh/id_rsa", "/container/key"])

    # Should fail with permission error
    if result == 1:
        output = capture.get()
        assert any(
            msg in output for msg in ["permission denied", "Push operation failed", "error"]
        )
    else:
...
def test_execute_permission_denied_destination(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/root/restricted"])
...
def test_execute_large_file_transfer(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/usr/bin/bash", "/container/bash"])
...
def test_execute_absolute_paths(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/passwd-copy"])
...
def test_execute_nested_directory_push(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/usr/share/doc", "/container/doc"])
    ...
    if result == 0:
        ...
        assert any(
            msg in output for msg in ["Directory pushed successfully", "Transfer completed"]
        )
    else:
        assert result == 1
...
def test_execute_file_metadata_preservation(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/passwd-meta"])
...
def test_execute_concurrent_push_operations(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/hosts", "/container/concurrent-hosts"])
...
def test_execute_network_interruption_handling(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/network-test"])

    # Should handle network issues gracefully
    if result == 1:
        output = capture.get()
        assert any(msg in output for msg in ["Push operation failed", "network", "error"])
    else:
        assert result == 0
...
def test_execute_disk_space_handling(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/usr/bin/bash", "/container/space-test"])
...
def test_execute_api_error_handling(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/api-test"])
...
def test_execute_progress_reporting(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/usr/bin/bash", "/container/progress-test"])
    ...
    if result == 0:
        ...
        assert any(
            msg in output for msg in ["File pushed successfully", "Transfer completed", "bytes"]
        )
    else:
        assert result == 1
...
def test_execute_checksum_verification(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/checksum-test"])
...
def test_execute_recursive_directory_push(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/systemd", "/container/systemd"])
...
def test_execute_transfer_cancellation(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/cancel-test"])
...
def test_execute_atomic_transfer(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/atomic-test"])
...
def test_execute_backup_existing_file(
    ...
    with command.shell.console.capture() as capture:
        result = command.execute(client=client, args=["/etc/passwd", "/container/backup-test"])
